...
private fun List<String>.toSchematicParts(): List<SchematicPart> =
    this.flatMapIndexed { y, value ->
            ...
            when {
                partChar.isDigit() -> {
                    currentPart = if(currentPart.toIntOrNull() != null || currentPart.isEmpty()) {
                        ...
                        val part = SchematicPart(Coordinate(x = currentX- currentPart.length, y = y), currentPart, type)
                        parts.add(part)
                    ...
                    currentPart = if(currentPart.toIntOrNull() != null) {
                        val part = SchematicPart(Coordinate(x = currentX - currentPart.length, y = y), currentPart, SchematicPartType.NUMBER)
                        parts.add(part)
                        "".plus(partChar)
                    } else {
                        val type = if (currentPart == ".") {
        ...
        when {
            ...
            currentPart == "." -> {
                val part = SchematicPart(Coordinate(x = currentX - currentPart.length, y = y), currentPart, SchematicPartType.NOTHING)
                parts.add(part)
            }
            else -> {
                val part = SchematicPart(Coordinate(x = currentX - currentPart.length, y = y), currentPart, SchematicPartType.SYMBOL)
                parts.add(part)
            }
        }
        parts
...
private fun Map<Coordinate, SchematicPart>.getPartNumbers(): List<Int> =
    this.entries
        ...
        .filter { schematicPart ->
            ...
            val coordinateEnd = Coordinate(x = coordinateStart.x + schematicPart.value.content.length - 1, y = coordinateStart.y)
...
private fun Map<Coordinate, SchematicPart>.getAdjacentParts(startCoordinate: Coordinate, endCoordinate: Coordinate): List<SchematicPart> {
    val xRange = startCoordinate.x - 1..endCoordinate.x + 1
    val yRange = startCoordinate.y -1..endCoordinate.y + 1
    return xRange.flatMap { x ->
        yRange.map { y ->
            if(x >= startCoordinate.x && x <= endCoordinate.x && y == startCoordinate.y) {
                null
            } else {
                val adjacentPartCoordinate = Coordinate(x, y)
                this[adjacentPartCoordinate]
            }
        }
    }.filterNotNull()
}
